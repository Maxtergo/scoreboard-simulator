...
from register import ReGister,REG_STATUS_HEADER
import os
...
class ScoreboardParser:
  ...
  def scoreboard_for_asm(self,asm_file):
...
class Scoreboard:
  ...
  def getdata(self):
    ...
    for unit in self.units:
      result += str(unit).replace("True"," Yes").replace("False","  No").replace("None","    ") + '\n'
      reg.write_back(unit.fi,unit.type)
    result+=REG_STATUS_HEADER+'\n'+str(reg);
    return result

""" It execute the cycle number given by the user """
def execute_cycle_number():
  i="void"
...
""" Clears the screen - this function is independent of Operating system """  
def clearscr():
  if(os.name == 'nt'):
	os.system('cls')
  else:
...
""" Print all the tables and with their current state """
def printAll():
  print(sb.printclock())
  print("\n                          INSTRUCTION STATUS")
  print("---------------------------------------------------------------------")
...
if __name__ == '__main__':
  sb = ScoreboardParser(ASM_FILE).scoreboard_for_asm(ASM_FILE)
  clearscr();
  m=printAll()-sb.clock+1
  while not sb.done():
    if(m<=0):
      m=1;
    sb.tick()
