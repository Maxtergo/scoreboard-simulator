...
class Instruction:
  ...
  def __init__(self, repr, op, dst, src1, src2,opt):
...
def __li(inst):
  ...
  opt = inst_toks[0]
  return Instruction(inst, op, fi, None, None,opt)
...
def __load_store(inst):
  ...
  opt = inst_toks[0]
  return Instruction(inst, op, fi, None, fk,opt)
...
def __arithmetic(inst):
  ...
  fk = inst_toks[3]
  opt = inst_toks[0]
  return Instruction(inst, op, fi, fj, fk,opt)
...
def __arithmetici(inst):
  ...
  fj = inst_toks[2]
  opt = inst_toks[0]
  return Instruction(inst, op, fi, fj, None, opt)
